...
The most significant changes are that it bumped the minimum Version of PHP to 8.3 and many classes got renamed. So updating could be cumbersome? Not really. As many people face the problems of upgrading code, some clever guys invented Rector, a tool for upgrading PHP with defined and tested rules, so-called rectors. This can help us to e.g. rename classes automatically or to convert code to newer constucts, e.g. `match `instead of `switch`, `string_starts_with` instead of checking `str_pos`, etc...
...
## Step 1: Update Code And All Modules To Lastest SS5
...
So now that you installed SS6 to your project, running dev/build will throw a bunch of errors. That's the time to upgrade your code. Enable `SilverstripeLevelSetList::UP_TO_SS_6_0` in your rector config and run rector.
...
For changing code in a thirdparty module you need to fork it on github.
